...
% q_f0 and q_delta by multi-start EM, and writes the EM-selected
...
if emResults.hitQFloor || emResults.hitQCeiling
...
title(ax, sprintf('Step 4 EM-selected CP-Control alpha-power difference (qF0 %.3g, qDelta %.3g)', ...
    emResults.q_f0_em, emResults.q_delta_em));
...
xlabel(ax, 'EM iteration');
ylabel(ax, 'Process noise q');
title(ax, 'Step 4 EM q histories');
legend(ax, 'Location', 'eastoutside');
set(ax, 'YScale', 'log');
style_ssm_axes(ax);

qHistoryPlotFile = fullfile(outDir, 'ssm_step4_em_q_history.png');
saveas(figQ, qHistoryPlotFile);
fprintf('Saved Step 4 EM q-history plot to %s\n', qHistoryPlotFile);
...
function summaryTable = build_multistart_summary(emResults)
    ...
    summaryTable = table(startMultiplier, initialQF0, initialQDelta, ...
        finalQF0, finalQDelta, finalLogLikelihood, emIterations, ...
        converged, logLikelihoodIsMonotone, hitQFloor, hitQCeiling, isBestStart);
end
