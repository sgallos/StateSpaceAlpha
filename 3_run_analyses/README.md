...
- `run_power_ssm_scalar_r.m`: power-band SSM workflow with one EM-fit total
  ...
  compares four q treatments, and writes trajectory, group-difference,
  q-version, EM-diagnostic, and Profile-q heatmap figures.
- `run_power_ssm_scalar_r_all_bands.m`: runs the scalar-`r` workflow for
  delta (1-4 Hz), theta (4-8 Hz), and alpha (8-12 Hz) using the collapsed
  multi-band sub-epoch table.
